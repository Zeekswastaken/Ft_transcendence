            ...
            <form>
                <div className="relative">
                    ...
                    <button type="submit" className="p-1 focus:outline-none focus:shadow-outline ">
                        {search.length  ? <p onClick={handleOnClickSearch}  className=' text-[20px] text-gray-500'>x</p>
                                        :   <img onClick={(e) => {e.preventDefault()}} src="/searchIcon.svg" width={20} height={20} alt="search" className=""/>}
                    </button>
                    </span>
                    <input autoComplete='off'  onChange={handleOnChangeSearch} value={search} type="search" name="q" className="  border-transparent focus:border-transparent focus:ring-0 w-[250px] h-[50px] py-2  text-sm text-[#6E4778] placeholder-[#6E4778] bg-[#411742]  rounded-xl pl-10 focus:outline-none focus:bg-primary-dark-500 focus:text-primary-white-200" placeholder="Search..." />
                </div>
            </form>
        </div> 
    );
