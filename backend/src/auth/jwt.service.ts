...
export class JWToken{
  constructor(private readonly jwtService: JwtService) {}
    ...
    async generateToken_2(user:Partial<User>):Promise<String>{
      ...
      const obj = {id:user.id,username:user.username,gender:user.gender,birthDay:user.birthDay,avatar_url:user.avatar_url,status:user.status,Socket:user.Socket,PlayerSocket:user.PlayerSocket,twofactorenabled:user.twofactorenabled};
